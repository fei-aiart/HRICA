...
# image_list = pd.read_csv('CelebA-HQ-to-CelebA-mapping.txt', delim_whitespace=True, header=None) # 第一列，CelebA-HQ数据标号，第二列CelebA中数据标号
# image_list = pd.read_table('CelebA-HQ-to-CelebA-mapping.txt',  sep='\s+') # 第一列，CelebA-HQ数据标号，第二列CelebA中数据标号
image_list = open('CelebA-HQ-to-CelebA-mapping.txt', 'r') # 第一列，CelebA-HQ数据标号，第二列CelebA中数据标号
f_train = open('train_list.txt', 'w')
f_val = open('val_list.txt', 'w')
f_test = open('test_list.txt', 'w')


# print(image_list[2,2] )
# i= 0
# for   x in enumerate(image_list.loc[:, :1]):
#     print (  x)
#     i=i+1
#     if i ==15:break
...
for line in lines[1:]:
    ...
    idx,x = line.split(' ')[0],line.split(' ')[1]
